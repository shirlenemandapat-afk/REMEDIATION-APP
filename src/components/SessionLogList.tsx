  ...
  return (
    <div className="space-y-4">
        ...
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-emerald-50 text-emerald-800 border border-emerald-200">
              ...
              Cross-Device Cloud Active
            </span>
          </div>
          <p className="text-xs text-slate-500 mt-0.5">
            Technology & Livelihood Education &bull; Sessions entered on this computer or any device are saved and synchronized across your account
          </p>
        </div>
      ...
      {filteredSessions.length > 0 ? (
        ...
        <div className="p-8 sm:p-10 text-center bg-white rounded-2xl border border-slate-200 text-slate-500 space-y-3">
          <div className="w-12 h-12 rounded-full bg-emerald-50 text-emerald-700 flex items-center justify-center mx-auto">
            <Calendar className="w-6 h-6" />
          </div>
          <div className="max-w-md mx-auto space-y-1">
            <p className="text-sm font-bold text-slate-800">No session records found</p>
            <p className="text-xs text-slate-500">
              If you just recorded sessions on another computer or device, click below to sync cloud records immediately.
            </p>
          </div>
          <div className="flex items-center justify-center gap-3 pt-2">
            {onManualSync && (
              <button
                ...
                disabled={isSyncing}
                className="px-4 py-2 bg-emerald-100 hover:bg-emerald-200 text-emerald-900 rounded-xl text-xs font-bold transition flex items-center gap-1.5 border border-emerald-300 shadow-xs cursor-pointer"
              >
                <RefreshCw className={`w-3.5 h-3.5 ${isSyncing ? 'animate-spin' : ''}`} />
                <span>{isSyncing ? 'Syncing...' : 'Sync Cloud Logs'}</span>
              </button>
            )}
            <button
              onClick={() => onOpenAddSession()}
              className="px-4 py-2 bg-emerald-800 hover:bg-emerald-700 text-yellow-300 rounded-xl text-xs font-bold transition flex items-center gap-1.5 border border-emerald-900 shadow-xs cursor-pointer"
            >
              <Plus className="w-3.5 h-3.5 text-amber-400" />
              <span>Add Daily Session</span>
            </button>
          </div>
        </div>
      )}
